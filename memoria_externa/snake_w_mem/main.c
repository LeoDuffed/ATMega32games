...
static void show_top_scores_wait(void){
    uint16_t scores[TOP_COUNT];
    ...
    lcd_draw_pattern_char(30, 0, T_);
    lcd_draw_pattern_char(36, 0, O_);
    lcd_draw_pattern_char(42, 0, P_);

    lcd_draw_number(8, 12, 1);
    lcd_draw_number(20, 12, scores[0]);

    lcd_draw_number(8, 21, 2);
    lcd_draw_number(20, 21, scores[1]);

    lcd_draw_number(8, 30, 3);
    lcd_draw_number(20, 30, scores[2]);

    lcd_draw_number(8, 39, 4);
    lcd_draw_number(20, 39, scores[3]);

    lcd_update();

    while(1){
        buttons_poll();
        if(any_button_pressed_event()){
            buttons_reset();
            return;
        }
